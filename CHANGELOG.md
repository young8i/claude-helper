...
- 修复 Windows 打包版安装中文补丁后安装日志仍需等待 Claude Desktop 手动关闭才显示成功的问题。
- 调整安装后的 Claude Desktop 重启流程，由主程序在安装脚本退出后异步打开，避免提权 PowerShell 被 Claude 进程拖住。

## v1.1.12 (2026-06-14)

### 修复

- 修复 Windows 安装中文补丁后需要手动关闭 Claude Desktop 才会结束安装日志的问题。
- 去掉 cc-switch 教程中的外部链接，改为引导用户使用工具内下载入口或商家提供的安装包。

## v1.1.11 (2026-06-14)

### 发布

- 重新发布安装包版本，确保包含 macOS 恢复脚本资源修复、Windows 黑窗口优化和大屏教程卡片样式修复。

## v1.1.10 (2026-06-14)

### 修复

- 修复 macOS 恢复原版时打包资源路径不稳定导致找不到 `install-mac.command` 的问题。
- 隐藏 Windows 普通命令行子进程窗口，减少检测、打开链接和安装流程中的黑窗口闪现。
- 修复大屏幕下第三方 API 配置教程卡片被强制拉伸导致底部空白的问题。
